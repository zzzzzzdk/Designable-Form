...
import { Modal, Button, message, Tabs, Input, Select } from 'antd';
...
export const DataSourceSetter: React.FC<IDataSourceSetterProps> = observer(
	(props) => {
		...
		const [dynamicConfig, setDynamicConfig] = useState<IDynamicConfigState>(() => {
			const maybe = (Array.isArray(value) && (value as any)[0]?.__dynamic__) || {};
			return {
				url: maybe.url || '',
				...
				>
					...
					>
						<Tabs
							...
							items={[
								{ key: 'static', label: <TextWidget token="SettingComponents.DataSourceSetter.staticTab" /> },
								{ key: 'dynamic', label: <TextWidget token="SettingComponents.DataSourceSetter.dynamicTab" /> },
							]}
						/>
						{mode === 'static' ? (
								...
								<div style={{ display: 'flex', gap: 12, flexDirection: 'column', padding: 12 }}>
									<label>
										...
										<Input
											...
											onChange={(e) => setDynamicConfig({ ...dynamicConfig, url: e.target.value })}
										/>
									</label>
									<label>
										<span>
										...
										<Select
											...
											onChange={(val) => setDynamicConfig({ ...dynamicConfig, method: val as any })}
										/>
									</label>
									<label>
										<span>
										...
										<Input
											...
											onChange={(e) => setDynamicConfig({ ...dynamicConfig, path: e.target.value })}
										/>
									</label>
									<label>
										<span>
										...
										<Input
											...
											onChange={(e) => setDynamicConfig({ ...dynamicConfig, labelKey: e.target.value })}
										/>
									</label>
									<label>
										<span>
										...
										<Input
											...
											onChange={(e) => setDynamicConfig({ ...dynamicConfig, valueKey: e.target.value })}
										/>
									</label>
									<label>
										<span>
										...
										<Input
											...
											onChange={(e) => setDynamicConfig({ ...dynamicConfig, childrenKey: e.target.value })}
										/>
									</label>
									{/* <label>
										<span>
